...
{
	public:
		void delete_vertice(vertice *,vertice *);
...
container_vertice* insert_cvertice(container_vertice *EV,container_vertice *v,atom *ATOM)
{
	int flag;
	...
	if(flag==1)
	{
		...
		{
	//		cout<<"everytime?\n";
			insert_cvertice(EV->next,v,ATOM);
		}
		else 
		{
			EV->next=v;
	...
	////////}
		...
		////////cout<<Atoms[Atoms[SAM].contigous[i]].x<<"\t"<<Atoms[Atoms[SAM].contigous[i]].y<<"\n";
			...
			add_connected(temp_vert_d,temp_vert_o,Atoms[SAM].bondinvoid[i]);
		        ...
		        if(Atoms[SAM].Cstart==NULL)
		        {
		        	Atoms[SAM].Cstart=new container_vertice;
		        	Atoms[SAM].Cstart->V=temp_vert_o;
		        }
		        else
		        {
		        	temp_cvert=new container_vertice;
		        	temp_cvert->V=temp_vert_o;
		        	insert_cvertice(Atoms[SAM].Cstart,temp_cvert,&Atoms[SAM]);
		        }
		        temp_cvert=new container_vertice;
		        temp_cvert->V=temp_vert_d;
			//cout<<Atoms[SAM].Cstart<<"\n";
			//display_SITE(Atoms[SAM].Cstart->V->p);
		        insert_cvertice(Atoms[SAM].Cstart,temp_cvert,&Atoms[SAM]);
		...
		////////}
			...
			////////cout<<Atoms[Atoms[SAM].contigous[i]].x<<"\t"<<Atoms[Atoms[SAM].contigous[i]].y<<"\n";
				...
				if(!s_temp)
				{
					s_temp=new vertice;
					s_temp->p=new site;
					s_temp->p->x=D_ONE->circum_x;
					s_temp->p->y=D_ONE->circum_y;
					s_temp->A=SAM;
					s_temp->D=D_ONE;
					temp_vert_o=s_temp;
				}
				else 
				{
					...
					temp->A=SAM;
					temp->D=D_ONE;
					temp=V->insert_vertice(s_temp,temp);
					//display_SITE(temp->p);
					temp_vert_o=temp;
				}
				//cout<<D_TWO->circum_x<<"\t"<<D_TWO->circum_y<<"\n";
